...
SPI::SPI(port_t port, uint32_t frequency, GPIO_TypeDef *GPIOx,
		uint32_t GPIO_Pin) :
		_csPort(GPIOx), _csPin(GPIO_Pin) {
...
void SPI::ConfigurePeripheral() {
	...
	if (!_hRes->configured) { // only configure periphiral once
		...
		_hRes->handle.Init.TxCRCInitializationPattern =
		SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
		_hRes->handle.Init.RxCRCInitializationPattern =
		SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
		_hRes->handle.Init.MasterSSIdleness = SPI_MASTER_SS_IDLENESS_00CYCLE;
		_hRes->handle.Init.MasterInterDataIdleness =
		SPI_MASTER_INTERDATA_IDLENESS_00CYCLE;
		_hRes->handle.Init.MasterReceiverAutoSusp =
...
void SPI::Write(uint8_t reg, uint8_t *buffer, uint8_t writeLength) {
	...
	if (HAL_SPI_TransmitReceive_IT(&_hRes->handle, txBuffer, rxBuffer,
			writeLength + 1) == HAL_OK) {
...
void SPI::TransactionWrite32(uint32_t value) {
	...
	uint8_t tx[4] = { (uint8_t)((value >> 24) & 0xFF), (uint8_t)(
			(value >> 16) & 0xFF), (uint8_t)((value >> 8) & 0xFF), (uint8_t)(
			(value) & 0xFF) };
	uint8_t rx[4] = { 0 };
