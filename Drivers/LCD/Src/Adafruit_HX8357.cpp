...
Adafruit_HX8357::Adafruit_HX8357(SPI *spi, IO *dcPin, IO *rstPin, uint8_t type) :
		Adafruit_SPITFT(
		HX8357_TFTWIDTH, HX8357_TFTHEIGHT, spi, dcPin, rstPin), displayType(
				type) {
}
...
HX8357B_SETPOWER, 3, 0x44, 0x41, 0x06,
HX8357B_SETVCOM, 2, 0x40, 0x10,
HX8357B_SETPWRNORMAL, 2, 0x05, 0x12,
HX8357B_SET_PANEL_DRIVING, 5, 0x14, 0x3b, 0x00, 0x02, 0x11,
HX8357B_SETDISPLAYFRAME, 1,
		...
		4,                     // undef2, 4 args
		0x40, 0x54, 0x26, 0xdb,
		HX8357B_SETGAMMA, 12, 0x00, 0x15, 0x00, 0x22, 0x00, 0x08, 0x77, 0x26,
		0x66, 0x22, 0x04, 0x00,
		HX8357_MADCTL, 1, 0xC0,
		HX8357_COLMOD, 1, 0x55,
		HX8357_PASET, 4, 0x00, 0x00, 0x01, 0xDF,
		HX8357_CASET, 4, 0x00, 0x00, 0x01, 0x3F,
		HX8357B_SETDISPMODE, 1, 0x00,   // CPU (DBI) and internal oscillation ??
...
HX8357D_SETC, 3, 0xFF, 0x83, 0x57, 0xFF, 0x80 + 300 / 5, // No command, just delay 300 ms
HX8357_SETRGB, 4, 0x80, 0x00, 0x06,
		...
		0x0D,                      // GDON
		0x78,                      // GDOFF
		HX8357D_SETGAMMA, 34, 0x02, 0x0A, 0x11, 0x1d, 0x23, 0x35, 0x41, 0x4b,
		0x4b, 0x42, 0x3A, 0x27, 0x1B, 0x08, 0x09, 0x03, 0x02, 0x0A, 0x11, 0x1d,
		0x23, 0x35, 0x41, 0x4b, 0x4b, 0x42, 0x3A, 0x27, 0x1B, 0x08, 0x09, 0x03,
		0x00, 0x01,
		HX8357_COLMOD, 1, 0x55,                      // 16 bit
		HX8357_MADCTL, 1, 0xC0,
		HX8357_TEON, 1, 0x00,                      // TW off
...
void Adafruit_HX8357::begin(uint32_t type) {
...
#endif
	startWrite();
	const uint8_t *addr = (this->displayType == HX8357B) ? initb : initd;
	...
	while ((cmd = pgm_read_byte(addr++)) > 0) { // '0' command ends list
		...
		if (!(x & 0x80))       // If not high bit set, issue args to command...
			while (numArgs--)
